...
group by 1 desc
order by 1;*/
        $query = "SELECT p.id_producto, correo, p.producto, SUM(c.cantidad) as cantidad, p.precio, p.imagen, SUM(c.cantidad * p.precio) as subtotal, u.id_usuario
        FROM carrito c
        JOIN producto p ON p.id_producto = c.id_producto
        JOIN usuario u ON u.id_usuario = c.id_usuario
...
";
    ...
    public function comprarCarrito($id_usuario)
    {
        ...
        foreach ($carritoCursos as $curso) {
            ...
            $query = "INSERT INTO detalle_venta (id_producto, id_usuario, fecha,cantidad,subtotal) VALUES (:id_producto, :id_usuario,  NOW(),:cantidad,:subtotal)";
            $rs = $this->db->prepare($query);
            $rs->bindParam(':id_usuario', $id_usuario, PDO::PARAM_INT);
